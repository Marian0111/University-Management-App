...
public class AdministratorPage extends JFrame implements ActionListener{
    int idAdministrator;
    JTable table1;
    JTable table2;
    JPanel panel = new JPanel();
    JPanel p2 = new JPanel();
    JLabel label = new JLabel("Administrator Page");
    ...
    int idMaterie;
    public AdministratorPage(int idAdministrator, Connection connection) {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        {
            f.dispose();
            AdministratorPage admin = new AdministratorPage(idAdministrator, connection);
        }
        ...
        {
            this.dispose();
            DatePersonale date = new DatePersonale(idAdministrator, connection, "administrator","administrator", idAdministrator);
        }
        ...
        if (e.getSource() == cautareBtn1)
        {
            ...
            {
                Error e3 = new Error("Selectati tipul de utilizator!");
            }
            if(idCautare != 0)
            {
                f.dispose();
                try {
                    ...
                    if (rs1.next())
                    {
                        if(studentRBTN.isSelected())
                        {
                            DatePersonale student = new DatePersonale(idCautare, connection,"student","administrator", idAdministrator);
                        }
                        else if(profesorRBTN.isSelected())
                        {
                            DatePersonale profesor = new DatePersonale(idCautare, connection,"profesor","administrator", idAdministrator);
                        }
                    }
                } catch (SQLException sqlex) {
                    System.err.println("An SQL Exception occured. Details are provided below:");
        ...
        if(e.getSource() == editare)
        {
            ...
            if(studentRBTN.isSelected())
            {
                try
                {
                    ...
                    {
                        f.dispose();
                        ModificareDate mod = new ModificareDate(rs1.getInt("idStudent"), connection, "student", "administrator", idAdministrator);
                    }
                    else
                    {
                        Error e1 = new Error("Student inexistent!");
            ...
            else if(profesorRBTN.isSelected())
            {
                try
                {
                    ...
                    {
                        f.dispose();
                        ModificareDate mod = new ModificareDate(rs1.getInt("idProfesor"), connection, "profesor", "administrator", idAdministrator);
                    }else
                    {
                        Error e2 = new Error("Profesor inexistent!");
                    }
    ...
    public void vizualizareUtilizatori()
    {
        ...
        studentRBTN.setBounds(100,130,100,30);
        studentRBTN.setBackground(Color.lightGray);
        profesorRBTN.setBounds(230,130,100,30);
